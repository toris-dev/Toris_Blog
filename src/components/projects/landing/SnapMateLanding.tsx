...
import { useCinematicReducedMotion } from './shared';
...
export default function SnapMateLanding({ project }: { project: Project }) {
  ...
  const reduceMotion = useCinematicReducedMotion();
  const activePhoto = captured
    ? {
        src: '/images/projects/snapmate/screen-gallery.png',
        alt: 'SnapMate에서 현상된 사진을 확인하는 그룹 갤러리 화면'
      }
    : {
        src: '/images/projects/snapmate/screen-camera.png',
        alt: 'SnapMate 카메라에서 촬영할 순간을 확인하는 화면'
      };
      ...
      signature={
        <SignatureFrame label="SnapMate 촬영 데모">
          <motion.div
            ...
            className={`cinematic-reduced-static mx-auto max-w-xs shadow-2xl ${
              captured
