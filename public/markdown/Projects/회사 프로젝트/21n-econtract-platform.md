...
> **Q. PG를 붙일 수 없는 규제 환경에서 계약·정산 시스템을 어떻게 설계했나?**
>
> PG 없이 KB국민은행 API로 계좌 조회·송금을, 법적 효력이 필요한 전자서명은 모두싸인 연동으로 처리해 계약·포인트·정산을 하나의 도메인으로 묶었다.
...
| 영역 | 스택 | 선택 이유 / 비고 |
| --- | --- | --- |
| 앱 | React Native, **[Expo](https://expo.dev)** | 한 명이 iOS·Android를 동시에 커버. `react-native-calendars`, Noto Sans KR(`@expo-google-fonts/noto-sans-kr`, `expo-font`) |
...
전자계약서·전자서명을 자체 구현하면 초기 비용은 아낄 수 있지만, 법적 효력·감사 추적·분쟁 대응까지 자체 부담이 됩니다. 얇은 개발 조직에서 감당할 리스크가 아니라고 판단해, [모두싸인](https://modusign.co.kr)의 기업 가입·인증·템플릿·API·웹훅까지 준비한 뒤 백엔드·앱·어드민에 녹였습니다. 연동 설계에서 지킨 원칙은 세 가지입니다.
...
| 결정 | 선택 | 이유(트레이드오프) |
| --- | --- | --- |
| 결제 구조 | PG 없이 KB국민은행 API(계좌 조회·송금) | 의료·미용·중개가 겹치는 도메인의 PG 심사 리스크 회피. 카드 결제 편의는 포기 |
| 전자서명 | 모두싸인 연동(자체 구현 배제) | 법적 효력·감사 추적·분쟁 대응 부담을 SaaS에 위임. 건당 과금과 외부 의존은 수용 |
| 저장소 구조 | 모노레포 + 최소 공유 원칙 | 앱·API·어드민의 타입 공유 편의 확보. 공유 모듈 실수의 파급 리스크는 최소 공유로 상쇄 |
| PDF 생성 | 큐 기반 비동기 처리 | Puppeteer의 시간·과금 부하를 API 응답에서 분리. 즉시 응답 대신 상태 알림 필요 |
| 외부 과금 관리 | 플래그 기반 단계적 롤아웃 | MVP 구간만 켜서 비용 곡선 통제. 설정 관리 복잡도는 증가 |
